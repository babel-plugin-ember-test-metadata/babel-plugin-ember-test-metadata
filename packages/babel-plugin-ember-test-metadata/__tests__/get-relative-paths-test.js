...
describe('get-relative-paths', () => {
  it('returns relative paths with the workspace prefix for classic projects', () => {
    const testCases = [
      {
        filePath: '/Users/tester/workspace/classic/extended/tests/acceptance/foo-test.js',
        expected: 'packages/extended/tests/acceptance/foo-test.js',
      }
    ];

    testCases.forEach(({ filePath, expected }) => {
      expect(_getRelativePathForClassicWorkspace(filePath, '/Users/tester/workspace/classic/', 'packages')).toEqual(expected);
    });
  });
